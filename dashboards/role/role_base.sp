graph "role_resource_structure" {
  ...
  node "rule" {
    ...
    sql = <<-EOQ
      select
        'rule' || (r -> 'verbs')::text || (r -> 'apiGroups')::text || (r -> 'resources')::text || (r -> 'resourceNames')::text as id,
        'rule' as title,
        jsonb_build_object(
        'Verbs', r -> 'verbs',
  ...
  edge "role_to_rule" {
    ...
    sql = <<-EOQ
        select
        uid as from_id,
        'rule' || (r -> 'verbs')::text || (r -> 'apiGroups')::text || (r -> 'resources')::text || (r -> 'resourceNames')::text as to_id
      from
        kubernetes_role,
        jsonb_array_elements(rules) as r
      where
