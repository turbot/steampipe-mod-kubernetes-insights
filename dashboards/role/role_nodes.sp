...
node "role_rule_resource" {
  category = category.role_rule_resource
...
///
node "role_rule_resource_with_name" {
  category = category.role_rule_resource //_name

  sql = <<-EOQ
    select
      concat('resource_name:',resource,':',resource_name) as id,
      concat(resource,':',resource_name) as title,
      jsonb_build_object(
        'resource', resource,
